	...
	QGraphicsView(parent), _pendown(false), _disabletablet(false)
{
    ...
    _pointerpath1->hide();
    _testscene->addItem(_pointerpath1);
...
//        _pointerpath1->setPen(QPen(Qt::red));
//        _pointerpath1->setPath(path1);
//        _pointerpath1->show();

        emit logEvent(QString("Element count: %1").arg(path.length()));
    }
//    if(event->)
}
...
void TabletTestView::mouseReleaseEvent(QMouseEvent *event)
{
    if(isMousePressed) {
        ...
        strokeCount++;
        emit logEvent(QString("strokes= %1").arg(strokeCount));
    }
	showMouseEvent(event);
}
...
void TabletTestView::showMouseEvent(QMouseEvent *event)
{
	...
	_pointertext->setText(
                QString("mouse %1 at %2, %3, buttons=[%4], click=%5")
				.arg(action)
				.arg(event->x())
				.arg(event->y())
				.arg(buttons)
